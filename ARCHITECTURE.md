...
**Schema V35** adds nullable/defaulted edition columns on `audiobook` (`edition_group_id`, `edition_group_optout`) plus indexes. Existing rows default into the grouping pass on the next scan/rescan; opting out keeps a specific edition visible as its own shelf card.

**Test coverage:** `LibraryServiceTests`, `SchemaV27Tests`, `SchemaV35Tests`, `AudiobookRecordLibraryFieldsTests`, `LibraryRootDAOTests`, `LibraryAccessTests`, `LibraryScannerTests`, `LibraryTitleResolutionTests`, `EditionMatcherTests`, `AudiolessEPUBCoverImportTests`, `LibraryViewModelTests`, `LibraryRootsViewModelTests`, `LibraryStatusMapTests`, `LibraryAvailabilityTests`, `LibraryLastBookRestoreTests`, and `SecurityScopeManagerLibraryRootTests`.
...
> **Build echo-cli with `make echo-cli` — never a bare `xcodebuild -scheme echo-cli`.** Two build-config traps hide here. (1) The scheme's default configuration is Debug, and a Debug (-Onone) binary is measurably slower: narrate is ONNX-inference-bound, so the measured gap there is ~26% wall for one real ~8-minute chapter (664 s → 493 s, which also folds in the 2→4 intra-op thread change) — but Swift-heavy commands (qa's TokenDTW matrices, EPUB parsing, deck/sidecar building) pay the full -Onone tax. The CLI now warns on stderr when it was built Debug, and `echo-cli --version` stamps the configuration. (2) Release's default **wholemodule** optimization miscompiles an async continuation in the narration chain on macOS 26 (Swift 6.2, MainActor-default isolation): the binary **hangs forever at the first synthesize** with zero scheduled tasks. Debug, Release+`-Onone`, and Release+`SWIFT_COMPILATION_MODE=incremental` all work; per-function `@_optimize(none)` pins do not help (it's cross-file codegen). The make target therefore pins `-configuration Release SWIFT_COMPILATION_MODE=incremental` (full per-function -O, no wholemodule) with a stable output path (`.build/cli/Build/Products/Release/echo-cli`). Re-test wholemodule after Xcode/toolchain updates.

**Headless performance & parallelism.** `narrate --jobs N` renders N chapters concurrently — each worker owns a private engine (ONNX session + voice packs, several hundred MB resident; 2–4 is the practical ceiling on a 16 GB machine) and claims chapters through a MainActor cursor, so markers/track IDs stay disjoint and GRDB's single writer serializes DB effects. `--threads` overrides the ONNX intra-op thread count per engine; the default now resolves from the machine's performance-core count on macOS (`NarrationEngineFactory.resolvedIntraOpThreads`, clamped 2…4 and divided across `--jobs`) instead of the historical iPhone-A14 pin of 2. The runner also calls `prepare(progress:)` up front, so the one-time 163 MB model download prints progress instead of looking hung; stderr progress is human-readable elapsed-stamped lines (`[3:12] chapter 3/12 · 26% of batch`, deduped on message), and the final `DONE` line reports audio-vs-wall realtime factor. Exit codes: 0 complete, 2 partial batch (re-run with `--resume`), 1 anything else. Omitting `--resume` on a work dir with prior captures now warns before wiping them. `narrate` holds a `ProcessInfo` activity (`idleSystemSleepDisabled`) for the whole run so overnight renders survive the Mac idling; `qa --classifier deterministic|auto` replaces the old hidden UserDefaults key and gates Foundation Models on *actual* `SystemLanguageModel` availability like the apps do.
...
**Visual listening mode** is an additive Now Playing surface for books with EPUB/PDF image blocks and aligned text. `VisualListeningCueResolver` is pure shared logic: it selects the current image cue, required subtitle block, karaoke word progress, and image timing policy (`beginning` or `midpoint`) from `EPubBlockRecord`, `TimelineItem`, and `WordTimingRecord` inputs. iPhone keeps the single-column listening flow and replaces album art only when the current snapshot has both an image and subtitle. iPad landscape uses a wider Now Playing composition with the visual stage beside compact metadata, the nudge copy, scrubber, and transport controls. macOS mounts `MacVisualStageView` above the persistent reader feed in `MacTriPaneView`, so the desktop experience shows the picture, subtitle, timing picker, book context, and player controls together. The feature is in-app only; it does not churn system Now Playing artwork and does not add a schema migration.
