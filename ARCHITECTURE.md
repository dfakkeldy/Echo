...
The seam is `protocol StudyDeckGenerating { func generate(sources:settings:) async -> GeneratedStudyDeckDraft }` (`Shared/Services/`), mirroring the narration-QA `DivergenceClassifier` pattern: `FixtureStudyDeckGenerator` (deterministic, always-available) and `AnthropicStudyDeckGenerator` (model-backed) both conform, and `StudyDeckGeneratorFactory.make(hasKey:anthropic:)` builds the network generator only when a key exists. `StudyDeckGenerationViewModel` injects the generator; `BookSettingsView`/`MacSettingsView` build it from `APIKeyStore` + the selected model (`AICardGenerationSettings`, default `claude-opus-4-8`, with a Sonnet/Haiku picker since the user bears token cost). Everything except the SwiftUI settings views is cross-platform (`Shared/`) and `nonisolated`, so the same pipeline already serves iOS and the planned on-device path.
