...
> **Update (2026-06-15 — on-device hardening, A14-verified).** Phase 0 on the iPhone 12 Pro confirmed the A14 crash is **fixed by stream-to-sink** (memory-dominated jetsam, not the vocoder) — narration sustains a long multi-chapter session, so the model swap is gated out (see `CODE_AUDIT_NARRATION.md` §3.1 + the model-swap plan's resolution banner). **Persistence:** chapters now render **once and persist** — the render loops (`PlayerModel+Narration.swift`) skip synthesis when the chapter file already exists, so reopen/export/per-item narration don't re-burn the ANE. Cache validity is keyed by `NarrationFileNaming.renderVersion` embedded in the filename — bump it when the rendered audio changes (DSP/rate/lead-out) and stale files regenerate once; `staleVoiceFiles` sweeps stale voices **and** orphaned older versions. **Audio quality:** `NarrationLowPass` (stateful RBJ Butterworth, fc 8.5 kHz) is applied to each sub-chunk before the ALAC write to tame the A14 vocoder's >9 kHz artifact (the "whine"); and the playback `AVAudioUnitTimePitch.overlap` is maxed (32) so 2× time-stretch doesn't smear into reverb. **Voice picker:** `VoicePickerView` is now reachable from Now Playing (it was dead UI). **Known gap:** FluidAudio's ANE Kokoro ships only the `af_heart` voice pack; the other catalog voices 404 on download until their `[510,256]` fp32 `.bin` packs are bundled.
