...
- [~] **Chapter Study Mode ("Anki mode" — chapter-as-card)** — book = deck; auto one chapter-card per chapter; listen → grade **Again / Good** (Again auto-default on no tap = hands-free); a due chapter with no cards = a **skippable re-listen**, and creating a flashcard inside a chapter prompts *"retire the chapter card and review with your cards?"*; **one interleaved FSRS queue**; **per-book + global** new-cards/day limits. **Progress:** generated listening assignments use only Again/Good, the queue can enforce a global daily new-chapter cap across active plans, Settings persists a clamped app-wide cap, and both study launch plus the dashboard review count use that cap. Remaining closeout: no-tap auto-Again, explicit skippable re-listen semantics, and retire-chapter prompt flow. *(Net-new — see spec Wedge 1 for the full design.)*
- [x] **AI-generated study cards** — book-text Q&A/cloze generation is now in the study system: BYO Anthropic key for the cloud path, plus an on-device Foundation Models provider when Apple's runtime is available. Cards are anchored to source blocks, validated before acceptance, and flow through the existing FSRS review pipeline. Remaining product polish is provider availability/copy/device QA, not the feature being deferred to 1.x.
- [x] **Card Inbox / mark-later**, full **card editor**, **decks & tags** (retire mid-playback popups). Mark-later capture routes to Card Inbox from phone controls, watch-configurable controls, and watch live commands; Card Inbox opens a prefilled full-card editor; existing cards can be edited from Decks; cards can be assigned to decks and tags; Decks supports manual create/rename/delete; inline mid-playback popups are retired. *(was WS6)*
- [x] **Narrator-audio-snippet cards** + **watchOS haptic review** — audio-snippet review is shipped; watch review now persists its due-card queue across eviction/relaunch, receives explicit empty-queue clears from iPhone, sends grades over the queued WatchConnectivity path, and centralizes haptic feedback behind the watch haptic preference.
...
- [x] **`.apkg` round-trip** + Markdown second-brain export — `.apkg` import/export now preserves decks, tags, scheduling state, and referenced media files across a round trip; the iOS More menu exports per-book Markdown `.zip` bundles, Settings exports a full-library study-notes `.zip`, and Echo Pro can continuously mirror text-only study Markdown to a user-picked folder with deterministic per-book files. *(was WS7)*
- [ ] **Deep analytics / full Insights** *(was 1.x → now 1.0)* — Study Insights now persist typed review metadata and show on-device daily review counts, retention curves, grade distributions, playback-speed trends, time-of-day patterns, and the 30-day due forecast; remaining work is the broader listening analytics surface (per-chapter coverage heatmap polish and a session-length definition decision). *(was WS3–4)*
- [ ] **Context Memory** (opt-in, on-device, deletable) — bookmark place tagging now has a visible Settings toggle, reduced-accuracy default, and one-tap deletion of bookmark places/session-location rows; session + chapter-start capture still need a product-scope decision before this can close. *(was WS5)*
...
- [x] **Restore CI test execution** — `.github/workflows/ci.yml` now runs on `macos-26`, resolves an iOS 26.4 `iPhone 17` simulator by UDID, compiles with `build-for-testing`, and executes `EchoTests` with `test-without-building` / parallel testing disabled so the suites gate every PR again. *(was WS2)*
- [~] **Crash-free gate:** **≥99.5% crash-free sessions (stretch 99.9%), zero unresolved P0 crash families**, measured via **MetricKit + App Store Connect** (no third-party SDK — keeps the no-tracking promise; symbolicate with `xcsym`). **Progress:** iOS and macOS now register a MetricKit subscriber and locally retain recent diagnostic JSON payloads in Application Support for privacy-preserving crash/hang review. Remaining closeout requires App Store Connect crash-free-session data, symbolication, and P0-family triage.
- [~] **No-lost-progress guarantee** — durable persistence + clean resume after call / eviction / relaunch, extended from the watch-persistence rigor to every surface. **Progress:** iOS scene-phase saves and crash-resilient playback-event heartbeats were already in place; macOS now persists the active track and playback position into App Group defaults and restores the matching track/position on file, folder, and narrated-book reopen. Remaining closeout is broader call/eviction/relaunch verification across iPhone, Watch, and Mac.
- [~] **Transcript/narration QA program (M1–M5)** — audio-only books can be transcribed into searchable reader text; source-backed books align ASR against canonical EPUB/PDF text; generated narration can be re-transcribed into reviewable divergences; pronunciation repair writes overrides and re-renders affected chapters; macOS parity is wired. Remaining closeout: finish the #338 remediation queue and device-level QA validation before promotion to `main`.
- [ ] **Performance budget** — fast cold launch, no playback freeze, smooth big-library scrolling. *(Narration TTS perf already addressed in PR #103: a cached `KokoroFrontEnd` stops re-parsing the ~6 MB lexicon per sub-chunk, and chapter-scoped word-timing materialization removes the O(chapters²) rebuild that made each chapter render slower than the last.)*
- [x] **A14 narration — ✅ resolved (merged from main).** The `OnnxKokoroEngine` ONNX pivot **shipped** (ONNX Runtime CPU EP, off the ANE); the A15+ `NarrationCapability` gate is **removed**, verified on an iPhone 12 Pro (≈0.7 s load, RTF ≈ 0.5, no crash/jetsam). See §A.1.
...
- [x] **UX audit first** — named the specific regressions across player / reader / library (incl. #77) in `docs/UX_AUDIT_2026-06-25_wedge3_clarity.md` before rebuilding.
- [ ] **UI overhaul** of the core surfaces to a coherent, obvious standard (owner sign-off that it's forward progress).
- [~] **Onboarding** that teaches the curb-cut workflow in under 60 seconds (import → align → capture → review). **Progress:** first-launch four-step walkthrough is wired; the Reader empty state now routes directly to choose a book or add an EPUB/PDF study document; and the visible companion-document importer accepts EPUB or PDF instead of contradicting Help/manual copy. Remaining closeout is a genuinely interactive tutorial that walks through the live import/alignment/capture/review actions.
- [~] **UX-flow audit** (no dead-ends/dismiss-traps; loading/empty/error states everywhere); clean navigation + library organization; reader speed controls; iOS/macOS parity. **Progress:** Reader and no-book Now Playing empty states now provide direct next actions, Reader search/filter misses now surface Clear Search / Show Everything recovery, Reader cards expose their long-press actions as VoiceOver custom actions, the Reader dock speed chip now adjusts playback speed inline from the app-wide presets while keeping a path to Playback Options, first launch no longer shows inert playback controls before content is loaded, Audiobookshelf browse now distinguishes library loading, per-library loading, empty libraries, and zero search results, Card Inbox and study-review launch failures now surface recovery instead of empty/no-op states, Help/manual/settings/architecture copy no longer points users to the removed Timeline tab or pre-speed-control Reader toolbar, stale audio-settings routes now open the real Playback Options surface instead of a placeholder, iOS/macOS Bookmark Loop controls now disable/explain the unavailable state until at least two enabled bookmarks can form a segment, Now Playing/Reader header utility buttons now meet 44pt touch targets, and the Phone/Watch player designers now have non-drag slot pickers for configurable controls. *(was WS9)*
...
- [x] **On-device Library (BYO-library shelf + folder roots)** — Schema V27, `LibraryService` / `LibraryScanner` / `LibraryAccess` / `LibraryRootDAO`, the iOS Library tab, smart landing, Browse By facets, status dots, Manage Roots, missing-file hide/relocate/remove flows, last-library-book restore, and macOS sidebar parity are shipped on this branch. See `docs/superpowers/specs/2026-06-26-local-library-design.md`.
- [x] **Full Audiobookshelf** *(was 1.x → now 1.0)* — auth (JWT login + refresh-with-rotation), browse libraries/items, server-side search, **download-to-local** (whole-item zip → `ABSLibrary/<remoteItemID>/`; sibling EPUB auto-discovered so alignment + flashcards fire unchanged), and two-way progress sync (ABS-authoritative last-write-wins reconciler, throttled push + reconcile-on-load). *Merged to `main` for the original iOS path (PR #102; V23 migration); nightly now adds macOS browse/import UI, multiple saved servers via V31, and macOS progress sync. Remaining fast-follows: device-verify live sync, full `URLSessionConfiguration.background` resumable download, and any multi-m4b progress edge cases; streaming deferred post-1.0.* *(was WS8b)*

### Wedge 6 — SUPPORT *(beats poor support / feedback)*

Competitors give slow, unhelpful support → **Echo is responsive and open.** In-app feedback path, responsive GitHub issues, the beta-tester funnel, living docs/manual/glossary as self-serve support. *(was WS10 — ongoing.)* **Beta funnel now live:** a TestFlight build + App Store metadata, a support/feedback email (`echo@kinnokilabs.com`), a public beta landing page (`docs/beta.html`), a tester guide shipped in PR #116/#117, and an in-app Feedback & Support surface on iOS and macOS with email, GitHub Issues, and manual links.

### Platform parity — macOS is a full peer in 1.0

**Shipped as one universal app** — the macOS target now shares the iOS bundle id (`com.echo.audiobooks`), so Echo is a **single App Store record (buy once → iPhone + Mac)** rather than a separate Mac app (PR #115). **Already built on Mac:** player, local Library sidebar, reader + karaoke, on-device alignment, Audiobookshelf browse/import/progress sync, Anki export, Card Inbox, Daily Review, PDF reader, Narration QA, and the **batch pipeline** (`MacBatchProcessingService` / `MacBatchQueueView`: import → transcribe → align → word-timings + overnight Narrate-EPUBs) — so **batch transcribe/align/narrate is largely shipped** (1.0 = complete + harden). **Parity gap 1.0 closes:** deeper Mac study-layer polish (Chapter-Study review ergonomics, decks/editor parity, deep Insights, Context Memory) plus applying the persisted custom font/theme (§A.1 follow-up). **Excluded** (platform-inherent, not gaps): watch review, CarPlay, camera bookmarks (Mac uses the photo library / files). macOS build specifics (sandbox, notarization, menus, AppKit bridging) → handled with `axiom-macos` at build time.
...
Photo-of-page → audio jump (the one row Echo is behind on; reuses the existing transcription index) · multi-voice / per-character narration · CarPlay capture buttons · ABS streaming (Part B §9.5) · AnkiConnect · `.apkg` export polish · focus-soundscape expansion · hyperfocus/transition alarms · Context Memory map view.
...
- [x] **Removed the unused FluidAudio + KokoroPipeline SPM packages** — dropped from `Echo.xcodeproj/project.pbxproj` (iOS + macOS) and `ThirdParty/KokoroPipeline` deleted; only `MisakiSwift` (G2P) and `onnxruntime` remain. *(Commit 373941a.)*
...
- [x] **PDF read-along + per-word Define & vocabulary (iOS, Schema V28)** — a text-parsed PDF gains a Page ⇄ Reflow reading surface (persisted per book); page mode auto-follows narration via the per-block `pdf_block_page` index and highlights the active word by PDFKit text search; tap-a-word seeks, press-hold offers Look Up + Save word; saved words become FSRS `vocabulary` cards reviewed with their context sentence. `ReaderSurfaceResolver`, `PDFReadAlongController`, `PDFBlockPageMapper`, `VocabularyCardBuilder`. *(Re-import existing PDFs once to populate page indices. M2/M3 iOS-only; macOS parity pending.)*
...
- [x] **Markdown / plain-text import (iOS + macOS)** — `.md`/`.markdown`/`.txt` files import as standalone narratable books via `TextDocumentParser` + shared `EPUBImportService.import(parse:)` + `DocumentImportFinalizer`; same narration, read-along, and chaptered playback as EPUBs. No schema change for this import path (reuses `epub_block`). *Future: attach text to an existing audio book for read-along; render-but-not-speak code blocks; image resolution; multi-file (folder-of-.md) books.*
- [ ] **PDF text import for narration** — `PDFAutoImportScanner` (PDFKit text extraction → shared `import(parse:)`/`DocumentImportFinalizer`) narrates a PDF's text; marker-less multi-page PDFs fall back to one synthetic chapter per page. **In:** headless runner / `echo-cli narrate` (accepts a `.pdf` file or folder) + iOS folder import. **GUI exposure (partial):** the iOS PDF reader now shows a **Narrate** button when a parsed PDF has no audio (one-tap on-device narration); the iOS document picker still lacks a PDF `UTType` and the macOS open panel / batch importer still reject PDFs, so import-time PDF→narrate isn't fully wired. No schema change for narration itself (reuses `epub_block`); the read-along page index lands separately in Schema V28.
- [x] **Chapter outline + tap-to-exclude (iOS)** — opening the playlist for a narration book now shows the **full EPUB chapter outline up front** (every narratable chapter, independent of render progress) via the pure `NarrationOutlineBuilder` → `NarrationOutlineChapter`. Tapping a chapter **excludes it from narration** (greyed + speaker-slash) so it is never synthesized or queued; built on the existing `is_hidden` block axis (`EPubBlockDAO.unhideChapter`), and already-rendered audio is kept on disk so re-including is instant. No schema change. *(PR #103.)*
- [x] **Read-first Listen UI polish** — iOS playback chrome now treats audio-less EPUB narration books as playback content: the root dock/mini-player and Read & Study toggle appear before rendered tracks exist, and play/toggle routes through the narration-aware `PlayerModel.play()` path.
- [x] **macOS custom font/theme application** — the macOS window now applies the persisted `appFont` and static `themeColor` tint, with the tri-pane player chrome and reader feed/card text using the configured accessibility font.
- [ ] **Hybrid streaming-start narration (latency parity vs Fox Reader)** — segment-planner, segment-cache filename/parser, cache completeness checks, in-memory segment readiness validation, renderer-level `renderSegmentFile` + persistent `renderSegment` primitives, no-gap segment audio behavior, a pure `NarrationSegmentAssembly` anchor/duration assembler, segment-aware `.m4b` export coalescing, durable `timeline_item.segment_key` reader scoping, and iOS player render/queue-by-segment orchestration are in place; segment-only cache chapters export in segment order while stamping one chapter marker per chapter, and full chapter files remain authoritative over same-chapter segments to avoid partial-cache exports. The remaining closeout is on-device verification/polish: confirm first-audio latency, no-gap segment transitions, resume behavior, free-tier chapter counting, and read-along across segment boundaries on real hardware. Competitor **Fox Reader** ships the *same* Kokoro model but starts near-instantly, even on an iPhone 12 Pro (A14). Evaluate a hybrid: **play the first chunk as soon as it's synthesized while rendering the rest ahead, then persist the finished AAC** so replays/exports/read-along keep the render-then-play battery+thermal win (the Voice Dream moat below). Concrete architecture (research-grounded — `docs/superpowers/research/2026-06-19-kokoro-a14-streaming-research.md`):
...
- [x] **Implement SM-2 core algorithm** — ✅ (historical) originally `SpacedRepetitionService.apply(grade:to:)`; **superseded in 1.0** by `FSRSScheduler`/`SM2Scheduler` behind the `SchedulingAlgorithm` seam (`FlashcardDAO.grade`), with FSRS now the live default (PR #97). `SpacedRepetitionService` no longer exists.
...
> Phase 0–5 work is merged into `nightly`; this section is retained as the historical checklist for the Read + Study merge.
...
- [~] **Reader toolbar speed controls** — add speed adjustment (at minimum) and loop mode to the reader-specific bottom toolbar. **Progress:** the shared Reader dock speed chip now opens an inline preset menu backed by `SettingsManager.Defaults.speedPresets` and calls `model.setSpeed` directly, while preserving a path to Playback Options for loop/skip settings. Remaining closeout is a product/design decision on inline loop placement.
...
- [x] **Replace inline flashcard popups with mark-later model** — inline flashcard popovers are retired; `InlineFlashcardTriggerController` and `FlashcardOverlayView` are deleted. Mark-later capture now routes passages to Card Inbox for intentional review and card creation instead of interrupting playback.
...
- [~] **Accessibility: VoiceOver audit (⬆️ P1 — elevated per Competitive Priorities §A.3)** — full pass through every screen with VoiceOver enabled. Prioritized because Voice Dream's loyal base is accessibility-driven; this is a competitive wedge, not a stretch goal. **Progress:** Reader EPUB paragraph/heading/image cards are now single accessible elements and expose the existing long-press actions as VoiceOver custom actions; Phone and Watch player designers no longer require drag-and-drop to configure slots. Remaining closeout requires live VoiceOver verification across all core screens.
...
- [x] **Settings UI** — Settings ▸ Library Sources (Connections screen, iOS) plus a native macOS Audiobookshelf sheet. iOS keeps a single active server flow; macOS supports multiple saved servers and switching the active server (Schema V31).
...
| WS2 | CI | ✅ Build gate + full EchoTests execution restored on pinned iOS 26.4 simulator |
| WS3–4 | Insights | 🟡 Partial (Study review charts, retention metadata, speed trend, and time-of-day charts shipped; session-length definition and chapter heatmap polish pending) |
| WS5 | Context Memory | 🟡 Partial (bookmark place tagging + Settings deletion shipped; session/chapter-start capture pending product decision) |
| WS6 | Anki core | 🟡 Partial (Card Inbox/decks/editor shipped; Chapter Study Mode pending product decision) |
| WS6b | Brain Dump / Book Notes | 🟡 Partial |
| WS7 | Import / Export | ✅ `.apkg` round-trip + Markdown second-brain export + text-only auto-export mirror shipped |
| WS-N | On-Device Narration (Kokoro) | 🟡 Core shipped; Listen UI + macOS theming done; hybrid streaming-start orchestration pending |
| WS8 | iCloud study sync | 🟡 Partial (anchors only) |
| WS8b | Audiobookshelf integration | ✅ Shipped to `main` for the original iOS path (PR #102; V23). Nightly adds macOS browse/import/progress sync plus multiple saved servers (V31). Fast-follows: device-verify live sync, full background-URLSession, multi-m4b progress edge cases; streaming post-1.0 |
...
| 9 | Audiobookshelf Integration | ✅ Shipped to `main` for the original iOS path (PR #102; connect/browse/search/download/progress-sync; V23); nightly adds macOS UI/progress sync and multi-saved-server support (V31). Live-sync device verification + streaming remain deferred/follow-up |

**Foundation (Phases 1–7): complete. Forward work tracked under Part A workstreams. Current schema head: V32 (fresh baseline V1 + additive migrations V25–V32).**
