...
Competitors give slow, unhelpful support → **Echo is responsive and open.** In-app feedback path, responsive GitHub issues, the beta-tester funnel, living docs/manual/glossary as self-serve support. *(was WS10 — ongoing.)* **Beta funnel now live:** a TestFlight build + App Store metadata, a support/feedback email (`echo@kinnokilabs.com`), a public beta landing page (`docs/beta.html`), and a tester guide shipped in PR #116/#117. *(Still open for the launch gate: an in-app feedback surface — no in-app `mailto`/feedback UI exists yet.)*
