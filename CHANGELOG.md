...
- **M4B chapter markers on export (iOS):** Combined-`.m4b` narration export now writes real chapter markers. The former copy-only `AudioMarker` stub is replaced by `ChapterMarkerWriter`, which authors Nero (`chpl`) + QuickTime (`chap`) atoms via the new `swift-audio-marker` dependency (pinned 0.1.1, iOS target only), and `NarrationExportService` labels chapters with their real `TrackRecord` titles. An `ExportProgressView` and an export entry in `NowPlayingTab` surface it. *Note:* AVFoundation's `loadChapterMetadataGroups` does not read these atoms back, so chapter markers are verified in a chapter-aware player (e.g. Books.app); the test suite asserts byte-level atom presence + playability.
