...
struct ABSServerDAO {
    ...
    /// Saves a newly authenticated server and activates it atomically. If activation fails,
    /// neither the new row nor changes to the previously active row are committed.
    func upsertAndSetActive(_ server: ABSServerRecord) throws {
        var copy = server
        try db.write { db in
            try copy.save(db)
            try db.execute(sql: "UPDATE abs_server SET is_active = 0")
            try db.execute(
                sql: "UPDATE abs_server SET is_active = 1 WHERE id = ?", arguments: [server.id])
        }
    }
