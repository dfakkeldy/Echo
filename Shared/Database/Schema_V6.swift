...
enum Schema_V6 {
    nonisolated static func migrate(_ db: Database) throws {
        ...
        try db.create(
            index: "idx_audiobook_added_at", on: "audiobook",
            columns: ["added_at"], unique: false, ifNotExists: true)
        try db.create(
            index: "idx_playback_state_last_played", on: "playback_state",
            columns: ["last_played_at"], unique: false, ifNotExists: true)
        try db.create(
            index: "idx_transcription_word_segment", on: "transcription_word",
