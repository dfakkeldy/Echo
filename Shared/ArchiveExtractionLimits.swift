...
enum ArchiveExtractionLimits {
    ...
    enum LimitError: LocalizedError, Equatable {
        ...
        var errorDescription: String? {
            switch self {
            case let .entryTooLarge(size, limit):
                "Archive entry is \(size) bytes, over the \(limit)-byte per-entry limit."
            case let .totalTooLarge(total, limit):
    ...
    static func checkedTotal(addingEntryOfSize entrySize: UInt64, to runningTotal: UInt64) throws -> UInt64 {
