...
struct EPUBTextNormalizationTests {
    ...
    @Test func hyperlinkDoesNotLeaveAnUnbalancedClosingTagInHTML() {
        // `<a>` is captured as a hyperlink marker, not emitted into HTML. The
        // close must be a no-op; previously it appended an orphan `</a>`.
        let xhtml = """
            <html xmlns="http://www.w3.org/1999/xhtml">
            <body><p>see <a href="u">link</a> here</p></body>
            </html>
            """
        let result = parseXHTML(from: Data(xhtml.utf8))
        let para = result.blocks.first { $0.kind == .paragraph }
        #expect(para?.htmlContent?.contains("</a>") != true)
        #expect(para?.text == "see link here")
    }
