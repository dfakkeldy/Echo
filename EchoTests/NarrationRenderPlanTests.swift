...
@Suite struct NarrationRenderPlanTests {
    ...
    @Test func emptyG2POutputSurvivesAsUntimedAdvisoryAuditEvidence() throws {
        let chunk = try PronunciationPlanner().planResolved("[record](/ɹəkˈɔɹd/)")
        let advisory = PronunciationAdvisoryEvidence(
            ...
            policyVersion: "audit-fixture-v1")
        let seed = try #require(PronunciationAuditContext.decisionSeed(
            for: PronunciationTokenEvidence(
                ...
                usedFallback: true),
            blockID: "empty-output",
            chunkDisplayText: "record",
            blockDisplayText: "record",
            wordBase: 0))
            .attachingAdvisoryEvidence(advisory)
        ...
        let manifest = PronunciationAuditManifest.make(
            ...
            captureCoverage: .complete,
            legacyChapterIndexes: [],
            audiobookURL: URL(fileURLWithPath: "/tmp/empty-output.m4b"),
            reelURL: nil,
            audiobookSHA256: String(repeating: "a", count: 64),
            listeningReelSHA256: nil,
            watchWords: ["record"],
        ...
        #expect(decision.selectedIPA.isEmpty)
        #expect(decision.kokoroTokenIDs.isEmpty)
        #expect(decision.advisoryEvidence == advisory)
        #expect(decision.chapterRelativeAudioRange == nil)
        #expect(decision.bookRelativeAudioRange == nil)
        #expect(decision.timingPrecision == nil)
        #expect(chunk.phonemes == "ɹəkˈɔɹd")
        #expect(materialization.diagnostics.map(\.reason) == [.decisionEvidenceMismatch])
        #expect(manifest.coverage == .incompleteEvidence)
        #expect(try JSONDecoder().decode(
            PronunciationAuditManifest.self,
            from: manifest.encoded()).decisions == [decision])
        #expect(PronunciationListeningReel.entries(
            decisions: [decision],
            audiobookURL: URL(fileURLWithPath: "/tmp/audit-only-fixture.m4b"),
            sourceDuration: CMTime(seconds: 30, preferredTimescale: 600)
        ).isEmpty)
    }

    @Test func unsupportedG2POutputStaysOutOfSynthesisButSurvivesAuditEvidence() throws {
        let chunk = try PronunciationPlanner().planResolved("[record](/ɹəkˈɔɹd/)")
        let advisory = PronunciationAdvisoryEvidence(
            category: .lexical,
            selectedAuthority: .uncertain,
        ...
        let manifest = PronunciationAuditManifest.make(
            ...
            captureCoverage: .complete,
            legacyChapterIndexes: [],
            audiobookURL: URL(fileURLWithPath: "/tmp/unsupported-output.m4b"),
            reelURL: nil,
        ...
        #expect(decision.bookRelativeAudioRange == nil)
        #expect(decision.timingPrecision == nil)
        #expect(!chunk.phonemes.contains("\u{0000}"))
        #expect(materialization.diagnostics.map(\.reason) == [.decisionEvidenceMismatch])
        #expect(manifest.coverage == .incompleteEvidence)
        #expect(try JSONDecoder().decode(
            PronunciationAuditManifest.self,
