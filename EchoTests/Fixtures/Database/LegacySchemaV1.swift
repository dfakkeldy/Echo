...
/// Frozen pre-squash V1 fixture used to build real historical upgrade starting points.
/// Source: parent of commit 9774be2f (`chore(db): reset GRDB schema baseline`).
enum LegacySchema_V1 {
    nonisolated static func migrate(_ db: Database) throws {
        ...
        try db.create(table: "transcription_word") { t in
            t.column("segment_id", .integer).notNull().references(
                "transcription_segment", onDelete: .cascade)
        ...
        // ── Indexes ──
        try db.create(
            index: "idx_track_audiobook_sort", on: "track", columns: ["audiobook_id", "sort_order"])
        try db.create(
            index: "idx_chapter_audiobook_sort", on: "chapter",
            columns: ["audiobook_id", "sort_order"])
        try db.create(
            index: "idx_bookmark_audiobook", on: "bookmark",
            columns: ["audiobook_id", "media_timestamp"])
        try db.create(
            index: "idx_flashcard_audiobook_due", on: "flashcard",
            columns: ["audiobook_id", "next_review_date"])
        try db.create(index: "idx_flashcard_due", on: "flashcard", columns: ["next_review_date"])
        try db.create(
            index: "idx_transcription_segment_audiobook", on: "transcription_segment",
            columns: ["audiobook_id", "start_time"])
        try db.create(
            index: "idx_playback_event_audiobook", on: "playback_event",
