...
/// A frozen, verbatim copy of the ordinary short-text **Landscape** rendering
/// path exactly as it existed at HEAD *before* Task 5's aspect-aware refactor
/// (background + aspect-fit figure + caption base, plus the inline
/// heard-wash / active-word subtitle band). It exists only so the renderer
/// tests can prove — at the same runtime, without a brittle checked-in PNG —
/// that the refactored `SlideshowFrameRenderer` produces byte-identical pixels
/// for ordinary Landscape image/caption/simple-subtitle and non-overflowing
/// karaoke frames. If the refactor drifts, the pixel-equality test fails.
///
/// This is deliberately NOT wired into production: it hard-codes the legacy
/// height-only geometry formulas and the old `width`/`height` initializer that
/// Task 5 removes from the shipping renderer.
nonisolated final class LegacyLandscapeFrameReferenceRenderer {
    ...
    init(
        ...
        imageLoader: @escaping @Sendable (String) -> CGImage? = {
            LegacyLandscapeFrameReferenceRenderer.loadImage(storedPath: $0)
        }
    ) {
        self.width = width
        self.height = height
