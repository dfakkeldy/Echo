    ...
    extension PlayerModel {
        ...
        private func buildWatchStateSnapshot() -> WatchStateSnapshot {
            ...
            // to embedded chapter markers (same source order as PlayerScrubberView).
            let playlistTicks = bookProgressBoundaryFractions
            s.bookBoundaryFractions =
                playlistTicks.isEmpty
                ? BookProgressTrackModel.tickFractions(
                    chapters: chapters, totalDuration: s.durationSeconds ?? 0)
                : playlistTicks

            // Crown volume mirror state
            s.outputGainDB = Double(watchCommandOutputGain)
            s.crownVolumeSensitivity = crownVolumeSensitivity

            // Settings (pre-resolved from SettingsManager)
            let settings = settingsManager
            s.crownAction = settings?.crownAction ?? SettingsManager.Defaults.crownAction
            s.isHapticFeedbackEnabled =
                settings?.isHapticFeedbackEnabled
                ?? SettingsManager.Defaults.isHapticFeedbackEnabled
            s.watchQuickBookmarkTimeoutSeconds =
                settings?.watchQuickBookmarkTimeoutSeconds
                ?? SettingsManager.Defaults.watchQuickBookmarkTimeoutSeconds
            s.seekBackwardDuration =
                settings?.seekBackwardDuration ?? SettingsManager.Defaults.seekBackwardDuration
            s.seekForwardDuration =
                settings?.seekForwardDuration ?? SettingsManager.Defaults.seekForwardDuration
            s.loopModeRawValue = loopMode.rawValue
            s.playbackSpeed = Double(speed)
            s.watchPage1Data =
                (try? JSONEncoder().encode(
                    settings?.watchPage1 ?? SettingsManager.Defaults.watchPage1)) ?? Data()
            s.watchPage2Data =
                (try? JSONEncoder().encode(
                    settings?.watchPage2 ?? SettingsManager.Defaults.watchPage2)) ?? Data()
            s.watchPage3Data =
                (try? JSONEncoder().encode(
                    settings?.watchPage3 ?? SettingsManager.Defaults.watchPage3)) ?? Data()
            s.watchPage4Data =
                (try? JSONEncoder().encode(
                    settings?.watchPage4 ?? SettingsManager.Defaults.watchPage4)) ?? Data()
            s.watchPage5Data =
                (try? JSONEncoder().encode(
                    settings?.watchPage5 ?? SettingsManager.Defaults.watchPage5)) ?? Data()
            s.linearBarMode = settings?.linearBarMode ?? SettingsManager.Defaults.linearBarMode
            s.linearBarHidden =
                settings?.linearBarHidden ?? SettingsManager.Defaults.linearBarHidden
            s.circularRingMode =
                settings?.circularRingMode ?? SettingsManager.Defaults.circularRingMode
            s.circularRingHidden =
                settings?.circularRingHidden ?? SettingsManager.Defaults.circularRingHidden
            s.watchArtworkLayout =
                settings?.watchArtworkLayout ?? SettingsManager.Defaults.watchArtworkLayout
            s.watchBackgroundStyle =
                settings?.watchBackgroundStyle ?? SettingsManager.Defaults.watchBackgroundStyle
            s.watchTitleScrollEnabled =
                settings?.watchTitleScrollEnabled
                ?? SettingsManager.Defaults.watchTitleScrollEnabled
            s.watchTitleScrollSpeed =
                settings?.watchTitleScrollSpeed ?? SettingsManager.Defaults.watchTitleScrollSpeed
            s.watchDateEnabled =
                settings?.watchDateEnabled ?? SettingsManager.Defaults.watchDateEnabled
            s.watchDateFormat =
            ...
            if let db = databaseService {
                ...
                do {
                    ...
                    os_log(
                        .error, "Failed to load due flashcards for watch: %{public}@",
