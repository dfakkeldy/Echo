...
| **4 · Trust** | ads, hidden fees, subscription lock-in | free core + optional Pro subscription or **lifetime** unlock, open-source, ad-free, **verifiably** private (you can read the code) |
...
Echo is a full-featured audiobook study application organized as a single Xcode workspace with four distinct targets. It supports bookmarking with optional voice memos, chapter navigation, loop modes, a sleep timer, variable playback speed, and intelligent rewind logic that adapts to pause duration. The iOS and watchOS apps communicate bidirectionally via WatchConnectivity, while a watchOS complication displays current artwork and progress. iPhone Lock Screen controls come from the system Now Playing integration, not the watch-only widget extension.
...
| **Echo Widget** (`watchOS`) | `Echo_WidgetBundle.swift` → `Echo_Widget.swift` | The watchOS widget extension exposes an `.accessoryCircular` complication with the current thumbnail and progress ring via the Watch App's `AppGroupDefaults`. Its VoiceOver value includes title, playback state, and progress. The circular family's accented/vibrant rendering uses the system-selected watch-face palette. The source includes an iOS Control Widget implementation behind a platform gate, but the current extension target is watchOS-only. |
...
- The circular Watch complication joins WidgetKit's system accent group, so its progress ring follows the watch-face palette; VoiceOver also announces title, playback state, and percentage so color is never the only signal.
